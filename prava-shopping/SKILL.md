...
Prefix every command with `PRAVA_SKILL_VERSION=<this skill's version>` (the `version:` in the
frontmatter) so the CLI can verify compatibility and stay quiet when you're current.
